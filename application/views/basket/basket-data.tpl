...
<tr class="data-giftItem alert">
    ...
    <td>
        ...
        <p class="b-gift">подарок</span></p>
        <p class="gift">{$item.gift.category} {$item.gift.brand} {$item.gift.name}</p>
    </td>
    <td></td>
    <td></td>
...
{/if}
    <tr>
      <td style="display: none;" class="data-transfer" colspan="7"></td>
...
<td colspan="7" style="padding: 0px;">
    ...
    <!--Вывод информации о доставке-->
      ...
      <td style="display: none;" colspan="7" class="cart-mini-summ">Сумма <span id="real_price_for_pay" class="data-tableSumm"></span> {$smarty.session.Currency.title}</td>
    </tr>
    </table>
...
$(document).ready(function() {
                      ...
                      function show_delivery_information() {
                        ...
                        if (testElements<1000 && {/literal} {$smarty.cookies.region} {literal}==1){
                          var testElementsnew = testElements-40;
                        document.getElementById('summ_za_vse').innerHTML = "Сумма: "+testElementsnew+"{/literal} {$smarty.session.Currency.title} {literal}";
                      } else{
