...
{elseif !empty($cart_item.img)}
    {assign var="cart_image" value="catalog/`$cart_item.img`"}
    {assign var="cart_image_extension" value=''}
{else}
...
data-img="{$url.img}/{$cart_image}{$cart_image_extension}"
data-price="{price $cart_item.price $cart_item.id_currency}" data-sale="10750" data-bonus="1500"
{if $cart_item.acttype=='gift'} data-gift-id="{$assocItem.id|default:''}" data-gift-name="Чайник ELECTROLUX Electro чайник" data-gift-img="/images/catalog/{gift assoc.id=$cart_item.actid}{/if}"