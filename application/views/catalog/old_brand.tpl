...
<div id="category_block_view_listing" class="span-12 no_margin">
        ...
        <div class="span-4 category_block_view_item">
            ...
            {if $item.acttype=='gift'}
                <div class="action_terms"><a href="#gift-modal-{$item.id}" data-toggle="modal"
                                             class="gift-txt-lnk data-getActInfo" id="i_{$item.id}">Условия акции<img
                                src="/assets/media/akcija.png"></a></div>
            {else}
    ...
    background-color: #5bde71;" href="#data-basket" onclick="addToCart({$item.id})" data-toggle="modal"
                           class="btn btn-info data-AddToCart data-itemId-{$item.id}"><i
                                    class="icon-shopping-cart icon-white"></i> <span>{if $item.id_availability == 1}Купить{else}Заказать{/if}</span></a>
...
{else}
    ...
    background-color: #f7a81e;" href="#data-basket" onclick="addToCart({$item.id})" data-toggle="modal"
                           class="btn btn-warning data-AddToCart data-itemId-{$item.id}"><i
