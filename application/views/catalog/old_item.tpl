...
<div itemscope itemtype="http://schema.org/Product">
    ...
    <div class="one_item_head">
        ...
        <h1>
            {if $item.category_onename}{$item.category_onename}{else}{$item.category}{/if} <span itemprop="brand"><span
                        itemprop="manufacturer" itemscope itemtype="http://schema.org/Organization"><span
                            itemprop="name">{$item.brand}</span></span></span>
            <span itemprop="name">{$item.name}</span>
        </h1>
        <a style="display:none" itemprop="url"
           href="{iurl assoc.parent=$item.parent_name assoc.cat=$item.cat_onename assoc.brand=$item.brand assoc.item=$item.name}"></a>
        ...
        {if $login == 'Vitaliy Shyshko'}
            ...
            <div class="like">
                <div class="like-item">
                    {literal}
                    <a rel="noindex, nofollow" href="https://twitter.com/share" class="twitter-share-button"
                       data-text="{/literal}{$item.category_onename} {$item.brand} {$item.name}{literal}. Интернет-магазин 590.ua"
                       data-url='https://590.ua/catalog/item/{/literal}{$item.id}{literal}' data-lang="ru">Твитнуть</a>
    ...
    {if ($login=='акция рождество' || $login=='Гальченко Владимир') && $item_id_temporary_celebration}
        <!-- Saint Nicolas -->
        <a href="https://goo.gl/4CLd9k" target="_blank"><img style="margin-top: -20px;margin-bottom: 5px;"
                                                             src="/images/hint/nikolay.gif"></a>
    {/if}
            ...
            <div class="span-4 one_item_img_block">
                ...
                {/literal}
                    ...
                    {foreach from=$item_id_temporary_cashback item=foo}
                        {if $item.id2 == $foo}
                            <a href="{$temporary_cashback_setting_action.action_link}" target="_blank"
    ...
    background-size: cover;" id="refund_electrolux"
                                     data-title="{$temporary_cashback_setting_action.item_image_info}"></div>
                            </a>
                            <a href="{$temporary_cashback_setting_action.action_link}" target="_blank"
                               title="Узнать подробнее...">
                                <div style="
                                        ...
                                        cursor: pointer;
                                        top: 191px;
                                        {if $item.acttype !== 'gift'&& $x3 > 0 || $item.id == '21465'}left: 10px;{else}left: 50px;{/if}">
                                    +{$temporary_cashback_summ[$iteration_action]}
                                    грн. {$temporary_cashback_setting_action.item_for_what}</div>
                            </a>
                    ...
                    {/foreach}
                {/if}
                <div class="main_photo">{if $item.images}{foreach from=$item.images item=item_image name=item_image}{if $smarty.foreach.item_image.first}
                        <a rel="prettyPhoto[pp]" href="{$url.img}/catalog/{$item_image.id}.{$item_image.imgext}"
    ...
    position: absolute;">
                ...
                <div style="
                    ...
                    {if $item.country}
                        {if $item.country == 'Германия' || $item.country == 'Німеччина' || $item.country == 'Germany' || $item.country == 'германия' || $item.country == 'німеччина' || $item.country == 'germany'}
                            <div class="country_icon"><img src="/images/icons/flags/Germany.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Germany</span>
                            </div>
                        {/if}
                        {if $item.country == 'Италия' || $item.country == 'Італія' || $item.country == 'Italy' || $item.country == 'italia' || $item.country == 'Italia' || $item.country == 'италия' || $item.country == 'італія' || $item.country == 'italy'}
                            <div class="country_icon"><img src="/images/icons/flags/Italy.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Italy</span>
                            </div>
                        {/if}
                        {if $item.country == 'Словения' || $item.country == 'Словенія' || $item.country == 'Slovenia' || $item.country == 'словения' || $item.country == 'словенія' || $item.country == 'slovenia'}
                            <div class="country_icon"><img src="/images/icons/flags/Slovenia.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Slovenia</span>
                            </div>
                        {/if}
                        {if $item.country == 'Словакия' || $item.country == 'словакия' || $item.country == 'Словаччина' || $item.country == 'словаччина' || $item.country == 'slovakia' || $item.country == 'Slovakia'}
                            <div class="country_icon"><img src="/images/icons/flags/Slovakia.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Slovakia</span>
                            </div>
                        {/if}
                        {if $item.country == 'Украина' || $item.country == 'Україна' || $item.country == 'Ukraine' || $item.country == 'украина' || $item.country == 'україна' || $item.country == 'ukraine'}
                            <div class="country_icon"><img src="/images/icons/flags/Ukraine.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Ukraine</span>
                            </div>
                        {/if}
                        {if $item.country == 'Франция' || $item.country == 'Франція' || $item.country == 'France' || $item.country == 'франция' || $item.country == 'франція' || $item.country == 'france'}
                            <div class="country_icon"><img src="/images/icons/flags/France.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">France</span>
                            </div>
                        {/if}
                        {if $item.country == 'Польша' || $item.country == 'Польща' || $item.country == 'Poland' || $item.country == 'польша' || $item.country == 'польща' || $item.country == 'poland'}
                            <div class="country_icon"><img src="/images/icons/flags/Poland.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Poland</span>
                            </div>
                        {/if}
                        {if $item.country == 'Сербия' || $item.country == 'Сербія' || $item.country == 'Serbia' || $item.country == 'сербия' || $item.country == 'сербія' || $item.country == 'serbia'}
                            <div class="country_icon"><img src="/images/icons/flags/Serbia.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Serbia</span>
                            </div>
                        {/if}
                        {if $item.country == 'Турция' || $item.country == 'Турція' || $item.country == 'Turkey' || $item.country == 'турция' || $item.country == 'турція' || $item.country == 'turkey'}
                            <div class="country_icon"><img src="/images/icons/flags/Turkey.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Turkey</span>
                            </div>
                        {/if}
                        {if $item.country == 'Литва' || $item.country == 'Lithuania' || $item.country == 'литва' || $item.country == 'lithuania'}
                            <div class="country_icon"><img src="/images/icons/flags/Lithuania.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Lithuania</span>
                            </div>
                        {/if}
                        {if $item.country == 'Таиланд' || $item.country == 'Таїланд' || $item.country == 'Thailand' || $item.country == 'таиланд' || $item.country == 'таїланд' || $item.country == 'thailand'}
                            <div class="country_icon"><img src="/images/icons/flags/Thailand.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Thailand</span>
                            </div>
                        {/if}
                        {if $item.country == 'Венгрия' || $item.country == 'Угорщина' || $item.country == 'Hungary' || $item.country == 'венгрия' || $item.country == 'угорщина' || $item.country == 'hungary'}
                            <div class="country_icon"><img src="/images/icons/flags/Hungary.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Hungary</span>
                            </div>
                        {/if}
                        {if $item.country == 'Румыния' || $item.country == 'Румунія' || $item.country == 'Romania' || $item.country == 'румыния' || $item.country == 'румунія' || $item.country == 'romania'}
                            <div class="country_icon"><img src="/images/icons/flags/Romania.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Romania</span>
                            </div>
                        {/if}
                        {if $item.country == 'Белоруссия' || $item.country == 'Белорусь' || $item.country == 'Білорусь' || $item.country == 'Belarus' || $item.country == 'белоруссия' || $item.country == 'белорусь' || $item.country == 'білорусь' || $item.country == 'belarus'}
                            <div class="country_icon"><img src="/images/icons/flags/Belarus.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Belarus</span>
                            </div>
                        {/if}
                        {if $item.country == 'Португалия' || $item.country == 'Португалія' || $item.country == 'Portugal' || $item.country == 'португалия' || $item.country == 'португалія' || $item.country == 'portugal'}
                            <div class="country_icon"><img src="/images/icons/flags/Portugal.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Portugal</span>
                            </div>
                        {/if}
                        {if $item.country == 'Болгария' || $item.country == 'Болгарія' || $item.country == 'Bulgaria' || $item.country == 'болгария' || $item.country == 'болгарія' || $item.country == 'bulgaria'}
                            <div class="country_icon"><img src="/images/icons/flags/Bulgaria.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Bulgaria</span>
                            </div>
                        {/if}
                        {if $item.country == 'Великобритания' || $item.country == 'Великобританія' || $item.country == 'United Kingdom' || $item.country == 'великобритания' || $item.country == 'великобританія' || $item.country == 'united kingdom'}
                            <div class="country_icon"><img src="/images/icons/flags/United-Kingdom.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">UK</span>
                            </div>
                        {/if}
                        {if $item.country == 'Южная Корея' || $item.country == 'Південна Корея' || $item.country == 'South Korea' || $item.country == 'южная корея' || $item.country == 'південна корея' || $item.country == 'south korea'}
                            <div class="country_icon"><img src="/images/icons/flags/South-Korea.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Korea</span>
                            </div>
                        {/if}
                        {if $item.country == 'Дания' || $item.country == 'Данія' || $item.country == 'Denmark' || $item.country == 'дания' || $item.country == 'данія' || $item.country == 'denmark'}
                            <div class="country_icon"><img src="/images/icons/flags/Denmark.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Denmark</span>
                            </div>
                        {/if}
                        {if $item.country == 'Хорватия' || $item.country == 'Хорватія' || $item.country == 'Croatia' || $item.country == 'хорватия' || $item.country == 'хорватія' || $item.country == 'croatia'}
                            <div class="country_icon"><img src="/images/icons/flags/Croatia.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Croatia</span>
                            </div>
                        {/if}
                        {if $item.country == 'Россия' || $item.country == 'Росія' || $item.country == 'Russia' || $item.country == 'россия' || $item.country == 'росія' || $item.country == 'russia'}
                            <div class="country_icon"><img src="/images/icons/flags/Russia.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">Russia</span>
                            </div>
                        {/if}
                        {if $item.country == 'Китай' || $item.country == 'КНР' || $item.country == 'China' || $item.country == 'CNR' || $item.country == 'Китайская Народная Республика' || $item.country == 'Китайська Народна Республіка' || $item.country == 'Republic of China' || $item.country == 'китай' || $item.country == 'кнр' || $item.country == 'china' || $item.country == 'cnr' || $item.country == 'китайская народная республика' || $item.country == 'китайська народна республіка' || $item.country == 'republic of china'}
                            <div class="country_icon"><img src="/images/icons/flags/China.png"
                                                           style="position: absolute;margin-left: -29px;margin-top: 1px;width: 25px;">made
                                in<br><span
                                        style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">China</span>
                            </div>
                        {/if}
                    {/if}

                    {if $item.bonus_amount>0}
                        <div onmouseover="hide_icons_bonus()" onmouseout="show_icons_bonus()" class="bonus_on_card"><img
                                src="/images/hint/piggy-bank.png"
                    ...
                    {if $item.id_availability == 1}
                        <div onmouseover="hide_icons_oplata_chast()" onmouseout="show_icons_oplata_chast()"
                             class="oplata_chast_icon">
                            ...
                            от <span
                                    style="color: rgba(255, 39, 2, 0.81);font-weight: bold;font-size: 10px;font-style: italic;">{$item.price/$rent_index*$curs_evro_smarty|round}</span><br><span
                    ...
                    {if $item.id_brand == '26' && $item.id_category != 60}
                        <div class="waranty_zirtal"><img src="/images/hint/waranty_zirtal.png"
                    ...
                    {if $item.bestprice > $item.price || $item.id2==41385}
                        <div onmouseover="hide_icons_best_price()" onmouseout="show_icons_best_price()"
                             class="best_price_icon"><img src="/images/hint/thumbs_up.png"
                ...
                {if $item.acttype !== 'gift'&& $x3 > 0 || $item.id == '21465'}
                    <div onmouseover="hide_icons_trade_in()" onmouseout="show_icons_trade_in()" class="trdein_icon"><img
                                src="/images/icons/zamena_catalog.png"
                ...
                {if ($login=='акция рождество' || $login=='Гальченко Владимир') && $item_id_temporary_celebration}
                    ...
                    {foreach from=$item_id_temporary_celebration item=foo}
                        {if $item.id == $foo && $item.acttype!='gift'}
                            <div onmouseover="hide_icons_black_friday()" onmouseout="show_icons_black_friday()"
                                 class="black_friday grow-rotate"><img class="pulse" src="/images/hint/black.png"
                                                                       style="position: absolute;margin-left: -29px;margin-top: 1px;">
                                <div style="
    color: #4aaa20;
    position: absolute;
...
">{$summ_temporary_celebration[$iteration_celebration]} грн.
                                ...
                                <div class="overlay_icons"
                                     style="margin-top: 60px;margin-left: -128px;font-style: italic;">
                                    <div class="wrapper_body">
                                        <div class="cbm_wrap ">
                ...
                {if $item.id_availability == 1}
                    ...
                    <div class="credit_logo_privatbank_pp" style="display: none;">
                        <img src="/images/icons/privatbank_pp_{if $item.rent >= 25}8{elseif $item.rent > 12.5}6{elseif $item.rent > 7.5}3{else}3{/if}_logo.png"
                             width="60"
                             title="Для 'Оплаты Частями' необходимо купить товар на сайте с помощью кнопки 'Купить'. Далее: в корзине сайта, при оформлении заказа, выбрать условия оплаты."
                             alt="Кредит Оплата Частями Приватбанк" rel="tooltip"
                             data-original-title="Для 'Оплаты Частями' необходимо купить товар на сайте с помощью кнопки 'Купить'. Далее: в корзине сайта, при оформлении заказа, выбрать условия оплаты.">
                    </div>
                    <div class="credit_logo_privatbank_ip" style="display: none;">
                        <img src="/images/icons/privatbank_ip_24_logo.png" width="60"
                             title="Для оплаты 'Мгновенной Рассрочкой' необходимо купить товар на сайте с помощью кнопки 'Купить'. Далее: в корзине сайта, при оформлении заказа, выбрать условия оплаты."
                             alt="Кредит Мгновенная Рассрочка Приватбанк" rel="tooltip"
                             data-original-title="Для оплаты 'Мгновенной Рассрочкой' необходимо купить товар на сайте с помощью кнопки 'Купить'. Далее: в корзине сайта, при оформлении заказа, выбрать условия оплаты.">
                    </div>
                    <div class="bonusplus_logo_privatbank" style="display: none;">
                        <img src="/images/icons/privatbank_bonusplus_logo.png" width="60"
                             title="Покупателям, оплатившим товар с помощью карточки ПриватБанка, зачисляется 'Бонус Плюс 5%'"
                             alt="Бонус Плюс Приватбанк" rel="tooltip"
                             data-original-title="Покупателям, оплатившим товар с помощью карточки ПриватБанка, зачисляется 'Бонус Плюс 5%'">
                    </div>
...
Скидка предоставляется в момент оплаты картами по сервису LiqPay.com" alt="Дисконтній клуб Плюс Приватбанк"
                             rel="tooltip" data-original-title="Скидкой по программе Дисконтный клуб могут воспользоваться только владельцы карт ПриватБанка уровня  Gold,Platinum или Infinite.
                    ...
                    {if $item.acttype !== 'gift'&& $x3 > 0 || $item.id == '21465'}
                        ...
                        <div class="zamena" style="display: none;">
                            <a href="/articles/zamena">
                                <img src="/images/icons/zamena-item.png" rel="tooltip"
                    ...
                    {if $login == 'Vitaliy Shyshko' && $item.images}
                        {foreach from=$item.images item=item_image name=item_image}
                            {if !$smarty.foreach.item_image.first}
                                <a rel="prettyPhoto[pp]" href="{$url.img}/catalog/{$item_image.id}.{$item_image.imgext}"
                                   title="{$item_image.brand} {$item_image.name}">
                                    <img itemprop="image"
                                         {if $item_image.id}src="{$url.img}/catalog/{$item_image.id}_s.{$item_image.imgext}"
                ...
                <div class="one_item_vblock_full {if !$item.rating}one_item_vblock_full_none_rate{/if}">
                    {if $item.rating}
                        <div class="one_item_rating" style="display: none;">
                            <p class="item-rate" itemprop="aggregateRating" itemscope
                               itemtype="http://schema.org/AggregateRating">
                                <meta content="{$item.rating|ceil}" itemprop="ratingValue"/>
                                <span itemprop="ratingCount" style="display: none">{$item.rating}</span>
                                <input name="star1" value="{$item.rating|round}" type="radio" disabled="disabled"
                                       {if $item.rating >= 1}checked="checked"{/if} class="star-item"/>
                                <input name="star1" value="{$item.rating|round}" type="radio" disabled="disabled"
                                       class="star-item" {if $item.rating|ceil >= 2}checked="checked"{/if} />
                                <input name="star1" value="{$item.rating|round}" type="radio" class="star-item"
                                       disabled="disabled" {if $item.rating|ceil >= 3}checked="checked"{/if} />
                                <input name="star1" value="{$item.rating|round}" type="radio" disabled="disabled"
                                       class="star-item" {if $item.rating|ceil >= 4}checked="checked"{/if} />
                                <input name="star1" value="{$item.rating|round}" type="radio" disabled="disabled"
                                       class="star-item" {if $item.rating|ceil >= 5}checked="checked"{/if} />
                            </p>
                            {literal}
                                <script type="text/javascript">
                    ...
                    {if !($item.archive == 1 && $item.id_availability == 3) }
                        <span class="one_item_status">
                                <span class="item-stock">
                                    {if $item.id_availability == 1}
                                        <strong class="in-stock"><link itemprop="availability"
                                                                       href="http://schema.org/InStock">Есть в наличии</strong>

{elseif $altItem.id_availability == 2 && $item.qrm>0}
                                        <a style="font-weight: bold;color: #0D8ABC;">Наличие уточняйте</a>

{elseif $altItem.id_availability == 2 && $item.qrm==0 || $item.qrm==""}
                                        <a class="on-order" target="_blank" href="/articles/4"
                                           title="Как получить товар под заказ">Под заказ</a>
...
{elseif $item.id_availability == 4}
                                        <strong class="in-stock">Есть в наличии, <br/>доставку уточняйте.</strong>
                                    {else}
                                        <a style="font-weight: bold;color: #0D8ABC;">Наличие уточняйте</a>
                                    {/if}
                                </span>
    ...
    color: #0d8abc;" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                    ...
                    {if $item.newmodel_id}<p>Заменен моделью <a
                                href="/catalog/item/{$item.newmodel_id}">{$item.newmodel_name}</a></p>{/if}
                        ...
                        <span>
                                <span class="one_item_old_price">
                                    <span class="prod_old_price"
                                          style="font-size: 23px;">{price $item.bdprice $item.id_currency} {$smarty.session.Currency.title}</span>
                                </span>
    ...
    color: #0d8abc;" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                                    <meta itemprop="price"
                                          content="{price $item.price}"/>{price $item.price $item.id_currency}
                    ...
                    {if $manager}
                        {if $item.rent}<p class="category-price">Категория
                            цены {if $item.rent >= 25}1{elseif $item.rent > 12.5}2{elseif $item.rent > 7.5}3{else}3{/if}</p>{/if}
                    {/if}
                    ...
                    {if $manager}
                        <div class="cat-manager-price">
                            <p>
                                {if $type_program == 'OR'}
                                    <strong style="padding: 5px 5px; background-color: #f8c278; color: #000; font-size: 11px;">Open
                                        Range</strong>
                                    <br>
                                {elseif $type_program == 'PR'}
                                    <strong style="padding: 5px 5px; background-color: #f8c278; color: #000; font-size: 11px;">Protect
                                        Range</strong>
                                    <br>
                                {elseif $type_program == 'PKR'}
                                    <strong style="padding: 5px 5px; background-color: #f8c278; color: #000; font-size: 11px;">Protect
                                        Kitchen Range</strong>
                                    <br>
                                {else}
                                    <strong style="padding: 5px 5px; background-color: #f8c278; color: #000; font-size: 11px;">{$type_program}</strong>
                                    <br>
                                {/if}
                                <span>РИЦ: {price $item.roznprice $item.id_currency} {$smarty.session.Currency.title}</span>
                            </p>
                            {if $item.discount>=0}<p>Скидка: - {$item.discount}%</p>{else}<p>Наценка:
                                + {math equation="x*y" x=$item.discount y=-1}%</p>{/if}
                            {if $login == 'Гальченко Владимир'}<strong>
                                Себестоимость: {if $item.cost==NULL}нет на складе{else}{$item.cost*$curs_evro_smarty}{/if}</strong>
                                <br>
                                Курс евро: {$curs_evro_smarty}
                                <br>
                                переменная:
                                <br>
                                {$rgrgregewrgewr|var_dump}{/if}
                            <p>{$item.qrm}, {$item.qty_z}
                                , {if $item.rqrm}{$item.rqrm}{else}-{/if}{if $login == 'Сергей Гринюк' || $login == 'ruslan'|| $login == 'Саша Ефимов' || $login == 'Vadyus' || $login == 'Vitaliy Shyshko' || $login == 'Гальченко Владимир'}{if $item.rent},({$item.rent}){/if},[{$item.sold}]{/if}</p>
                    ...
                    {if !($item.archive == 1 && $item.id_availability == 3)}
                        <div class="item-compare span-5 one_item_afterimg">
                            <span style="position: absolute;{if $action && $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01 && $item.bonusAmount <= 0 && $item.id_availability != 4}margin-left: 187px;margin-top: -97px;{elseif $action && $item.bonusAmount <= 0 && $item.id_availability != 4}position: absolute;margin-left: 187px;margin-top: -67px;{elseif $action && $item.id_programm==3}position: absolute;margin-left: 447px;margin-top: -117px;{elseif $action && $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01}margin-top: -125px;margin-left: 187px;{elseif $action}margin-top: -95px;margin-left: 187px;{else}{if $item.bonusAmount > 0}margin-top: -97px;margin-left: 447px;{elseif $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01}margin-top: -95px;margin-left: 447px;{elseif $item.id_availability == 4}margin-top: -95px;margin-left: 447px;{else}margin-top: -66px;margin-left: 447px;{/if}{/if}"
                                  title="Добавить в список сравнения" id="comp_{$item.id}" data-id='{$item.id}'
                                  data-category='{$item.cat|mblower}'
                                  class="one_item_compare data-toggleCompare{if in_array($item.id, $compareItems)} checked{/if}"
                                  {foreach from=$item.images item=item_image name=item_image}{if $smarty.foreach.item_image.first}data-url="{$url.img}/catalog/{$item_image.id}_s.{$item_image.imgext}"{/if}{/foreach} {if $action}style="position: absolute;"{/if}></span>
                            <span id="compare_option"
                                  style="position: absolute;{if $action && $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01 && $item.bonusAmount <= 0 && $item.id_availability != 4}margin-left: 184px;margin-top: -63px;{elseif $action && $item.bonusAmount <= 0 && $item.id_availability != 4}position: absolute;margin-left: 184px;margin-top: -31px;{elseif $action && $item.id_programm==3}margin-top: -81px;margin-left: 444px;{elseif $action && $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01}margin-left: 184px;margin-top: -92px;{elseif $action}margin-left: 184px;margin-top: -61px;{else}{if $item.bonusAmount > 0}margin-top: -61px;margin-left: 444px;{elseif $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01}margin-top: -60px;margin-left: 444px;{elseif $item.id_availability == 4}margin-top: -60px;margin-left: 444px;{else}margin-top: -32px;margin-left: 444px;{/if}{/if}font-size: 10px;line-height: 12px;">Сравнить</span>
                        </div>
                    {/if}
                    <a style="{if $item.archive && $item.id_availability == 3}display: none !important;{/if}{if $action}{if $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01}position: absolute;margin-left: 187px;margin-top: -46px;{else}position: absolute;margin-left: 187px;margin-top: -16px;{/if}                                                     ;margin-left: 389px;margin-top: -97px;{elseif $item.archive}position: absolute;margin-left: 387px;margin-top: -70px;{else}position: absolute;margin-left: 389px;margin-top: -69px;{/if} {if ($login == 'Гальченко Владимир' || $login == 'Сергей Гринюк') && !$action}position: absolute;margin-left: 447px;margin-top: -136px;{/if};font-weight: bold;color: #0D8ABC;display: block;float: left;"
                       href="#lp" data-toggle="modal" title="Уведомить о снижении цены"><img
                                style="width: 40px;height: 35px;" src="https://590.ua/images/watch_price.png"><br><span
                                id="watch_price"
                                style="position: absolute;left: -1px;{if $action}top: 32px;{else}top: 36px;{/if}width: 43px;font-size: 10px;line-height: 12px;">Следить<br>за ценой</span></a>
                    {if $action}
                        <a style="{if $item.specprice  && ($item.bdprice > $item.price) && ($item.bdprice - $item.price)/$item.price > 0.01}position: absolute;margin-left: 189px;margin-top: 18px;{else}position: absolute;margin-left: 189px;margin-top: 48px;{/if}font-weight: bold;color: #0D8ABC;display: block;float: left;"
                           href="#comments-modal" data-toggle="modal" title="Мы ценим Ваше мнение"><img
                                    style="width: 35px;height: 32px;"
                                    src="https://590.ua/images/restfeedback.png"><br><span id="watch_price"
                                                                                           style="position: absolute;left:-4px;top: 32px;width: 43px;font-size: 10px;line-height: 12px;text-align: center;">Оставить<br>отзыв</span></a>
                    {else}
                        <a style="{if $item.archive && $item.id_availability == 3}display: none !important;{/if}position: absolute;{if $item.id_availability == 4}margin-left: 449px;margin-top: -40px;{elseif $item.archive}margin-left: 449px;margin-top: -17px;{else}margin-left: 449px;margin-top: -17px;{/if}font-weight: bold;color: #0D8ABC;display: block;float: left;"
                           href="#comments-modal" data-toggle="modal" title="Мы ценим Ваше мнение"><img
                                    style="width: 35px;height: 32px;"
                                    src="https://590.ua/images/restfeedback.png"><br><span id="watch_price"
                                                                                           style="position: absolute;left:-4px;top: 32px;width: 43px;font-size: 10px;line-height: 12px;text-align: center;">Оставить<br>отзыв</span></a>
                ...
                <div style="{if $action}{else}display: block;position: absolute;left: 620px;width: 130px;{/if}">
                    ...
                    <br>
                    <span {if $action}{else}style="margin-left: 17px;"{/if}><span
                                style="font-size: 16px;color: #509b5d;font-weight: bold;"
                            ...
                            {if !in_array($item.id, $basketStats.ids)}
                                {if $item.brand == 'Miele'}
                                    ...
                                    {if $action}margin-bottom: 10px;{else}margin-top: 10px;{/if}
                                            ...
                                            background-color: #5bde71;" href="#data-basket"
                                       onclick="addToCart({$item.id})" data-toggle="modal"
                                       data-producterommerceid="{$item.id}"
                                       class="btn btn-info data-AddToCart data-itemId-{$item.id}" data-placement="top"
                                       title="Для преобретения техники Miele, Вам необходимо созвониться с торговым менеджером для оформления заказа. Телефон: +38(050)590 0 590"><i
                                                class="icon-shopping-cart icon-white"></i> <span>Купить</span></a>
                                {else}
                                    <a style="float: left;
                                    {if $action}margin-bottom: 10px;{else}margin-top: 10px;{/if}
                                            background-image: linear-gradient(to bottom, #5bde71, #2ba247);
                                       ...
                                       onclick="addToCart({$item.id})" data-toggle="modal"
                                       data-producterommerceid="{$item.id}"
                                       class="btn btn-info data-AddToCart data-itemId-{$item.id}"><i
                                                class="icon-shopping-cart icon-white"></i> <span
                                                style="font-size: 17px;">Купить</span></a>
                                {/if}
                            {else}
                                <a style="
                                        float: left;
                                {if $action}margin-bottom: 10px;{else}margin-top: 10px;{/if}
                                        ...
                                        background-color: #f7a81e;" href="#data-basket" onclick="addToCart({$item.id})"
                                   data-toggle="modal" class="btn btn-warning data-AddToCart data-itemId-{$item.id}"><i
                                            class="icon-shopping-cart icon-white"></i> <span>В корзине</span></a>
                            {/if}
            ...
            <div class="one_item_additional_block">
                <ul class="one_item_additional_ul">
                    ...
                    {if $item.instruction}
                        <li>
                            <a class="li_instruction" href="/files/instruction/{$item.id}.{$item.instruction}">Скачать
                                инструкцию</a>
                        </li>
                    {/if}
                ...
                <ul class="one_item_additional_ul">
                    ...
                    <li>
                        ...
                        <span>
                                <img style="position: absolute;height: 30px;margin-top: -126px;margin-left: 57px;"
                                     src="/images/icons/item/visa-mastercard.png">
                            </span>
                        <div style="margin-top: -20px;">
                            <img style="height: 45px;" src="/images/icons/item/bonus-plus.png">
                            <img title="Дисконтный клуб ПриватБанка"
                                 style="height: 41px;margin-left: 5px;position: absolute;margin-top: 2px;"
            ...
            {if $item.id == '14957'}
                <div class="item-article">
                    Холодильник Electrolux EN 4000 AOW по праву заслужил позитивные отзывы и популярность среди
                    украинских покупателей благодаря доступной цене, оптимизированному внутреннему пространству, удобным
                    полкам и большому объему – 375 л. при высоте 2 м. Купить холодильник Электролюкс EN4000AOW в
                    интернет-магазине 590.ua сейчас еще выгоднее – цена снижена до конца августа!
                    ...
                    <div class="sale_complect_block span-12 no_margin">
                        <div class="sale_complect_content">
                            {foreach from=$set.items item=itemset name=itemsets}
                                <div class="sale_complect_item"
                                     onclick="javascript:window.location.href='{iurl assoc.subdomain=$itemset.subdomain assoc.cat_latin=$itemset.cat_latin_single assoc.brand=$itemset.brand assoc.item=$itemset.name}'">
                                    <div class="sale_complect_photo">
                                        <a href="{iurl assoc.subdomain=$itemset.subdomain assoc.cat_latin=$itemset.cat_latin_single assoc.brand=$itemset.brand assoc.item=$itemset.name}"
                                           title="{$itemset.brand} {$itemset.name}">
                                            <img src="{$url.img}/catalog/{$itemset.imgid}_s.{$itemset.imgext}"
                                                 alt="Фото {$itemset.brand} {$itemset.name}"/>
                                        </a>
                                    </div><!-- // sale_complect_photo -->
                                    <a href="{iurl assoc.subdomain=$itemset.subdomain assoc.cat_latin=$itemset.cat_latin_single assoc.brand=$itemset.brand assoc.item=$itemset.name}"
                                       title="{$itemset.brand} {$itemset.name}"
                                       class="one_item_complect_title sale_complect_title">{$itemset.brand} {$itemset.name}</a>
                        ...
                        <div class="sale_complect_footer">
                            ...
                            <a class="sale_complect_buy" href="#data-basket" onclick='addToCart({$set.id},false,"set")'
                               data-toggle="modal">Купить комплект</a>
    ...
    {if $assocItems|@count > 0 && !$item.archive}
        ...
        <div id="slider_asoc_aviable"
             style="position: relative; margin-bottom: 20px; box-sizing: border-box; overflow: hidden;{if $electrolux_action==1 && !$action}margin-top: 30px;{/if}">
            <div class="product_box">
                <div class="product_box_title" style="text-align: left;">
                    ...
                    <br><span style="font-size: 13px;color: #0d8abc;">мы для вас подготовили <span
                                style="font-size: 13px; color: #ef6e7a;">{$countVmeste} {$correctFormVmeste}</span></span>
                </div>
            </div>
                                ...
                                <div>
                                        ...
                                        <div class="ls-info">
                                            <div class="ls-title">
                                                ...
                                                <div style="position: fixed; top: 100px;">
                                                    ...
                                                    <span style="font-size: 14px;display: block;color: #ffffff;border-radius: 5px;padding: 5px;background-image: linear-gradient(to bottom, #eba4a4, #9d4141);">{price $item.price $item.id_currency}
                                                        грн</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="ls-info">
                                            <div class="ls-title">
                                                <a href="{iurl assoc.parent=$assocItem.parentname assoc.subdomain=$assocItem.subdomain assoc.cat_latin=$assocItem.cat_latin_single assoc.brand=$assocItem.brand assoc.item=$assocItem.name}"
                                                   style="color:#0d8abc">{$assocItem.cat_onename} {$assocItem.brand} {$assocItem.name}</a>
                                                <div style="position: fixed; top: 100px;">
                                                    <!--<span style="font-size: 15px; display: block; color: green;">
                                                    ...
                                                    <span style="font-size: 14px;display: block;color: #ffffff;border-radius: 5px;padding: 5px;background-image: linear-gradient(to bottom, #eba4a4, #9d4141);">{price $assocItem.price $assocItem.id_currency}
                                                        грн</span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="ls-total">
                                        ...
                                        <div class="alert alert-info"
                                             style="position: absolute;  padding: 10px; margin-top: -36px;">
                                            <span style="text-decoration: line-through;text-align: center;display: block;font-size: 20px;color: #868686;margin-top: 0px;">{math equation="(x+y)*$curs_evro_smarty|round" x=$item.price y=$assocItem.price}
                                                грн</span>
                                            ...
                                            <span style="margin-top: 10px;color: #fa5662;display: block;text-align: center;"><span
                                                        style="color: black;">Вы экономите</span>
                                        ...
                                        {/if}
                                            ...
                                            <span style="display: block;text-align: center;margin-top: -3px;color: #509b5d;font-weight: bold;"><span
                                                        style="color: black;"><!--От --></span>
                                                {if $item.rent >= 25}
                                                    {if $assocItem.rent >= 25}
    ...
    " href="#data-basket" onclick="addToCart({$item.id});addToCart({$assocItem.id});" data-toggle="modal"
                                                       data-producterommerceid="{$item.id}"
                                                       class="btn btn-info data-AddToCart data-itemId-{$item.id}"><i
                                                            class="icon-shopping-cart icon-white"></i> <span>Купить комплект</span></a>
                                            </center>
                ...
                <div class="product_icon" data-qtip="1">
                    ...
                    <div class="product_icon">
                        ...
                        <div class="prev_bonus" style="display:none;">
                            Купив {$assocItem.brand} {$assocItem.name} у нас, Вы получаете
                            +{$assocItem.bonus_amount|round}грн на бонусный счет к цене!
                        </div>
                        <a href='{iurl assoc.parent=$assocItem.parentname assoc.subdomain=$assocItem.subdomain assoc.cat_latin=$assocItem.cat_latin_single assoc.brand=$assocItem.brand assoc.item=$assocItem.name}'
                           class="product_icon_title">{$assocItem.brand} {$assocItem.name}</a>
                        ...
                        {if $assocItem.acttype=='gift'}
                            <a href="#gift-modal-{$assocItem.id}" data-toggle="modal"
                               class='one_item_gift data-getActInfo' id="i_{$assocItem.id}">
                                        <span class='one_item_gift_img'>
                        ...
                        <div class="price_info">
                            ...
                            {if $assocItem.specprice  && ($assocItem.bdprice > $assocItem.price) && (($assocItem.bdprice - $assocItem.price)/$assocItem.price > 0.01)}
                                ...
                                <div class="action_price">
                                    <span>{price $assocItem.price $assocItem.id_currency}</span>
                                    <span>{$smarty.session.Currency.title}</span>
                                </div>
                            {else}
                                <div class="price">
                                    <span style="color: #0d8abc;">{price $assocItem.price $assocItem.id_currency}</span>
                                    <sup style="color: #0d8abc;">{$smarty.session.Currency.title}</sup>
                                </div>
                            {/if}
                            {literal}
                                <style type="text/css">
    ...
    margin-right: 15px;" data-toggle="modal" href="#data-basket" onclick="addToCart({$assocItem.id})"
                               class='one_item_blue_button prodbox_buy_button data-AddToCart data-itemId-{$assocItem.id}'>
                                Купить</a>
                        </div><!-- // price_info -->
        ...
        {if $item.brand == 'Electrolux'}
            ...
            <script type="text/javascript" src="//media.flixfacts.com/js/loader.js" data-flix-distributor="12386"
                    data-flix-language="u2" data-flix-brand="Electrolux"
                    data-flix-mpn="{php}echo str_replace(' ', '', $this->item->name);{/php}" data-flix-ean=""
                    data-flix-sku="" data-flix-button="flix-minisite" data-flix-inpage="flix-inpage"
        ...
        <div class="tabs">
            <ul id="itemTab" class="nav nav-tabs">
                ...
                {if $fields}
                    <li class="data-primary"><a href="#fields" data-toggle="tab">Характеристики</a></li>
                {/if}
                {if $item.images|@count > 1}
                    <li><a href="#gallery" data-toggle="tab">Фото</a></li>
                {/if}
                {if $video.type=='youtube'}
                    <li><a href="#video" data-toggle="tab">Видео</a></li>
                {/if}
                {if $similarItems|@count > 0}
                    <li><a href="#similarItems" data-toggle="tab"><span class='strong'>Похожие модели</span></a></li>
                {/if}
                <li><a href="#comments" data-toggle="tab">Отзывы {if $comments|@count > 0}({$comments|@count}){/if}</a>
                </li>
            ...
            <div class="tab-content" itemprop='description'>
                                            ...
                                            <tr>
                                                <td class="field-name">
                                                    {if !empty($_field.description)}
                                                        <span class="data-field" rel="popover"
                                                              data-original-title="{$_field.name}"
                                                              data-content="{$_field.description|htmlspecialchars}"
                                                              data-placement="right">{$_field.name}</span>
                                                    {else}
                                                        {$_field.name}
                                                    {/if}
                                                </td>
                        ...
                        {foreach from=$similarItems item=altItem name=similarItems}
                            <div class="tab-item">
                                <div class="tab-item-img">
                                    {if $altItem.imgid}
                                        <a href="{iurl assoc.subdomain=$item.subdomain assoc.parent=$altItem.parentname assoc.cat_latin=$altItem.cat_latin_single assoc.brand=$altItem.brand assoc.item=$altItem.name}">
                                            <img {if $altItem.imgid}src="{$url.img}/catalog/{$altItem.imgid}_s.{$altItem.imgext}"{else}
                                                 src="/{$module}/images/noImg.png"{/if}
                                                 alt="{if $altItem.cat_onename}{$altItem.cat_onename|mblower}{else}{$altItem.cat|mblower}{/if} {$altItem.brand} {$altItem.name} купить"
                                                 title="{if $altItem.cat_onename}{$altItem.cat_onename|mblower}{else}{$altItem.cat|mblower}{/if} {$altItem.brand} {$altItem.name}"/>
                                        </a>
                                    {/if}
                                </div><!-- // .tab-item-img -->
                                ...
                                <div class="tab-item-info">
                                    ...
                                    <div class="tab-item-info-brand"><a
                                                href="{iurl assoc.subdomain=$item.subdomain assoc.parent=$altItem.parentname assoc.cat_latin=$altItem.cat_latin_single assoc.brand=$altItem.brand assoc.item=$altItem.name}"
                                                title="{$altItem.brand} {$altItem.name}" rel="tooltip"
                                                data-placement="top"
                                                data-original-title="{$altItem.brand} {$altItem.name}">{$altItem.brand} {$altItem.name}</a>
                                    </div>
                                        ...
                                        {if !($altItem.archive == 1 && $altItem.id_availability == 3)}
                                            ...
                                            {elseif $altItem.id_availability == 2 && $item.qrm==0 || $item.qrm==""}
                                                <a class="on-order" target="_blank" href="/articles/4"
                                                   title="Как получить товар под заказ">Под заказ</a>
                                            {/if}
                                            {if $altItem.id_availability == 3}
                                                <a class="on-order" target="_blank" href="/articles/4"
                                                   title="Как получить товар под заказ">Под заказ</a>
                                            {/if}
                                            {if $altItem.id_availability == 4}
                                                <strong class="in-stock" rel="tooltip" data-placement="top"
                                                        data-original-title="Есть в наличии. Доставка через 5 рабочих дней"
                                                        title="Есть в наличии. Доставка через 5 рабочих дней">Есть в
                                                    наличии. Доставка через 5 рабочих дней</strong>
                                            {/if}
                                        {/if}
                                    ...
                                    <div class="tab-item-info-buy"><a href="#data-basket"
                                                                      onclick="addToCart({$altItem.id});"
                                                                      data-toggle="modal"
                                                                      class="btn btn-info">Купить</a></div>
                                </div><!-- // .tab-item-info -->
                            </div>
                            <!-- // .tab-item -->
                        {/foreach}
                    </div>
                    <!-- // .tab-pane #similarItems -->
                {/if}
                ...
                <div class="tab-pane fade" id="comments">
                    <div class="one_item_reviews_head">
                        ...
                        <a href="#comments-modal" class="one_item_blue_button one_item_reviews_button"
                           data-toggle="modal">Оставить отзыв</a>
                    </div>
            ...
            <script type="text/javascript">
                ...
                if ((thisDate < endDate) && (startDate < thisDate)) {
                    if (randomInteger(0, 1) == 1) {
                        setTimeout(function () {
                            ...
                            $.jGrowl("Сейчас этот товар просматривают<br><strong>" + viewers_count + " " + viewers_count_desc + "</strong>", {
                                position: 'bottom-left',
                                header: '<strong>Пользуется спросом!</strong>',
                                sticky: true
                            });
                        }, 6000);
                    }
                ...
                if (get_id_region() == 'Киев') {
                    if (friday_num == 6 || friday_num == 7) {
                        if (randomInteger(0, 1) == 1) {
                            $.jGrowl("Можем доставить эту модель во <span style='font-weight:bold;'>вторник</span> в течении дня по городу <strong>" + get_id_region() + "</strong>, <strong>при заказе на выходных или до 17:00 в понедельник</strong>", {
                                position: 'bottom-left',
                                header: '<strong>Быстрая и удобная доставка</strong>',
                                sticky: true
                            });
                        }
                    } else if (friday_num == 5) {
                        if (randomInteger(0, 1) == 1) {
                            $.jGrowl("Можем доставить эту модель в <span style='font-weight:bold;'>понедельник</span> в течении дня по городу <strong>" + get_id_region() + "</strong>, <strong>при заказе сегодня до 17:00</strong>", {
                                position: 'bottom-left',
                                header: '<strong>Быстрая и удобная доставка</strong>',
                                sticky: true
                            });
                        ...
                        if (randomInteger(0, 1) == 1) {
                            $.jGrowl("Можем доставить эту модель <span id='timer_delivery_day' style='font-weight:bold;'>завтра</span> в течении дня по городу <strong>" + get_id_region() + "</strong>, <span id='timer_delivery'></span>", {
                                position: 'bottom-left',
                                header: '<strong>Быстрая и удобная доставка</strong>',
                                sticky: true
                            });
                ...
                if (randomInteger(0, 1) == 1) {
                    setTimeout(function () {
                        $.jGrowl("Это самая низкая цена, которую вы видели в городе <strong>" + get_id_region() + "</strong>", {
                            position: 'bottom-left',
                            header: '<strong>Вам повезло!</strong>',
                            sticky: true
                        });
                    }, 12000);
                }
