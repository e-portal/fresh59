...
<div class="span-16 prod_compare_block no_margin">
  <table>
    ...
    <tr>
      ...
      {foreach from=$items item=item} 
        <td><a href="{iurl assoc.parent=$item.parentname assoc.subdomain=$item.subdomain assoc.cat=$item.cat_onename assoc.brand=$item.brand assoc.item=$item.name}"><img {if $item.imgid} class="data-zoom" src="{$url.img}/catalog/{$item.imgid}_s.{$item.imgext}" data-url="{$url.img}/catalog/{$item.imgid}.{$item.imgext}"{else} src="/{$module}/images/no-image.jpg"{/if} data-alt="Фото {if $item.cat_onename}{$item.cat_onename}{else}{$item.cat}{/if} {$item.brand} {$item.name}" /></a></td>
      {/foreach}
    </tr>
    <tr>
      <td>Модель</td>
      {foreach from=$items item=item} 
        <td><a href="{iurl assoc.parent=$item.parentname assoc.subdomain=$item.subdomain assoc.cat=$item.cat_onename assoc.brand=$item.brand assoc.item=$item.name}" class="compare_title">{$item.brand} {$item.name}</a></td>
      {/foreach}
    </tr>
    <tr>
      <td>Цена</td>
      {foreach from=$items item=item}
        <td>
          <span class="one_item_price complect_current_price"><span class="prod_price">{price $item.price $item.id_currency} {$smarty.session.Currency.title}</span></span>
          <div class="clear"></div>
          {if !in_array($item.id, $basketStats.ids)}
            <a href="#data-basket" onclick="addToCart({$item.id})" data-toggle="modal" class="one_item_blue_button prodbox_buy_button compare_buy_button data-AddToCart data-itemId-{$item.id}">&nbsp;Добавить в корзину</span></a>
          {else}
            <a href="#data-basket" onclick="addToCart({$item.id})" data-toggle="modal" class="one_item_blue_button prodbox_buy_button compare_buy_button btn-warning data-AddToCart data-itemId-{$item.id}">&nbsp; Уже в корзине</a>
          {/if}
        </td>
      {/foreach}
    </tr>
