            ...
            {foreach from=$item_id_electrolux_action item=foo}
                {if $item.id2 == $foo}
                    <span {if $item.acttype=='gift'}id="refund_electrolux_gift" {else}id="refund_electrolux"{/if}
                          data-title="Повертаємо гроші для оплати комунальних послуг" style="display: block;
                            text-align: center;
                            color: #f9ebf3;
                            font-size: 16px;
                            font-weight: bold;
        ...
        {if $catId == '28' || $catId == '29'}
            <div class="span-4 category_block_view_item">
                ...
                {foreach from=$item_id_electrolux_action item=foo}
                    {if $item.id2 == $foo}
                        <span {if $item.acttype=='gift'}id="refund_electrolux_gift" {else}id="refund_electrolux"{/if}
                              data-title="Повертаємо гроші для оплати комунальних послуг" style="display: none;
                                text-align: center;
                                color: #f9ebf3;
                                font-size: 16px;
                                font-weight: bold;
    ...
    <div class="span-4 category_block_view_item" data-qtip="1">
        ...
        <div class="span-4 category_block_view_item">
            ...
            <style type="text/css">
                ...
                .black_friday_{/literal}{$number_of_element}{literal} {
                    margin-top: 185px;
                    margin-left: 160px;
                    position: absolute;
                    z-index: 4;
    ...
    background-color: #f7a81e;" href="#data-basket" data-productErommerceId="{$item.id}" onclick="addToCart({$item.id})"
                ...
                {literal}
                    <div style="{/literal}{if $login == 'Гальченко Владимир'}display:none;{/if}{literal}
                        ...
                        text-align: center;
                        margin-top: -25px;
                        ">от {/literal}{$item.price/$rent_index*$curs_evro_smarty|round}{literal}
                    <br>
                    грн./мес</div>
                {/literal} {/if}
