      ...
      {foreach from=$brandTree key=parent item=children name=tree}
        <ul>
            ...
            {foreach from=$children item=child key=key}
              <li><a href="/catalog/cat/{$key}/brand/{$brand.id}">{$child} <span class='strong'>({$counts[$key]})</span></a></li>
            {/foreach}
        </ul>
      {/foreach}
    </div>
...
<div class="trdein_icon_{$number_of_element}"><img src="/images/icons/zamena_catalog.png" style="position: absolute;margin-left: -29px;margin-top: 1px;"><div class="overlay_icons" style="margin-top: -460px;margin-left: -201px;font-style: italic;">
    <div class="wrapper_body"><div class="cbm_wrap ">
          ...
          {if $item.acttype=='gift'}
            <div class="gift">
              <div class="products_container_image_wrap">
                <a href="#gift-modal-{$item.id}" data-toggle="modal" class="gift-txt-lnk data-getActInfo" id="i_{$item.id}"><img src="/images/catalog/{gift assoc.id=$item.actid}" alt=""></a>
              </div>
              <span>Подарок</span>
            </div>
          {/if}
...
{/if}
    ...
    {if !$brand}
      <br />
      <div class="alert alert-error">Найдено <span class="strong">{$pages->totalItemCount}</span> записей.</div>
    {/if}
...
{else}
  ...
  <div class="alert alert-error">
    ...
    <br />
    {if $maybe}
      Возможно, вы имели ввиду <a class="link" href="https://590.ua/catalog/search/{$maybe|htmlspecialchars}">{$maybe}</a>
    {/if}
    <br />
