...
<body>
{literal}
<!-- Yandex.Metrika counter --> <script type="text/javascript" > (function (d, w, c) { (w[c] = w[c] || []).push(function() { try { w.yaCounter173141 = new Ya.Metrika({ id:173141, clickmap:true, trackLinks:true, accurateTrackBounce:true, webvisor:true }); } catch(e) { } }); var n = d.getElementsByTagName("script")[0], s = d.createElement("script"), f = function () { n.parentNode.insertBefore(s, n); }; s.type = "text/javascript"; s.async = true; s.src = "https://cdn.jsdelivr.net/npm/yandex-metrica-watch/watch.js"; if (w.opera == "[object Opera]") { d.addEventListener("DOMContentLoaded", f, false); } else { f(); } })(document, window, "yandex_metrika_callbacks"); </script> <noscript><div><img src="https://mc.yandex.ru/watch/173141" style="position:absolute; left:-9999px;" alt="" /></div></noscript> <!-- /Yandex.Metrika counter -->{/literal}
...
<div class="main_block holder_{$zf.params.controller}_{$zf.params.action}{if $manager} holder_logged{/if}">
    ...
    <div class="span-16 site_panel">
        <div class="site_panel_left_side">
            ...
            {if $login}
                <span class="autorization_link autorization_name"><a href="/auth"><span class='strong'>{$login}</span></a> / <a href="/auth/exit">Выйти</a></span>
            {else}
    ...
    <div class="span-16 logo">
        ...
        <div class="middle">
            <div class="top">
                <div class="social_links">
                    <a rel="nofollow" href="https://www.facebook.com/590.ua" target="_blank"><img src="/assets/media/fb.png" width="20" height="20" alt="fb"></a>
                    <a rel="nofollow" href="http://vk.com/ua590" target="_blank"><img src="/assets/media/vk.png" width="20" height="20" alt="vk"></a>
        ...
        <div class="contacts cities">
            <div class="region-choose">
                ...
                <div class="btn-group">
                    <span class="btn btn-mini city-button" data-toggle="dropdown">
                        <span class="data-current-city">
                            {foreach from=$regions item=region}{if $region.active}<span id="this_city_name" style="font-weight: bold;">{$region.name}</span>{/if}{/foreach}
                                <i class="caret"></i>
                        </span>
                    </span>
                    <ul class="dropdown-menu data-region-list">
                        {foreach from=$regions item=region}
                            <li><a href="#" id="region_{$region.id}" data-phone='{$region.phone}' onclick="selectRegion({$region.id})">{$region.name}</a></li>
                        {/foreach}
                    </ul>
                </div>
                <div class="phones">
                    <span class="data-phones ringo-phone">{foreach from=$regions item=region}{if $region.active}
                        {if $region.id==1}<span style="font-size: 15px;float: left;line-height: 19px;margin-left: 70px;"><img style="width: 16px;height: 16px;position: absolute;top: 31px;left: 50px;" src="/images/icons/vodafone.png">(050) 590-0-590<br><img style="width: 18px;height: 18px;position: absolute;top: 49px;left: 49px;" src="/images/icons/phone.png">(044) 223-8-520<br><img style="width: 16px;height: 16px;position: absolute;top: 67px;left: 50px;" src="/images/icons/kievstar.png">(067) 590-0-591</span>{else}<span>{$region.phone}</span>{/if}{/if}{/foreach}</span>
                </div>
                {foreach from=$regions item=region1}{if $region1.active}
                {if $region1.id==1}
                <div class="days">
                    <p style="float: right;margin-right: 30px;margin-top: -3px;">пн - чт: 9:30 - 18:00<br>пт-вс: 10:00 - 18:00<br>Call центр</p>
    ...
    <div class="span-16 top_menu">
        ...
        <div class="basket">
            <a id="data-basket-order"  {if $zf.params.action=='order' || $zf.params.action=='paybycard' || $zf.params.action=='thankyou'}style="pointer-events: none;cursor: default;color: #999;"{/if}  href="#data-basket" class="basket_info {if $basketStats.amount > 0}active {/if}data-openBasket shopping-cart-lnk">
    ...
    <div class="span-16 central_block">
        <div class="span-4 sidebar">
                ...
                <ul class="action" style="display: none;">
                    <li><a href="/catalog/sets">Акционные комплекты<img src="/assets/media/star.png" width="45" height="35" alt="star"></a></li>
                </ul>
                <ul class="toggle_sidebar">
                    <li><a href="javascript:void(0)" onclick="$('.sidebar ul.sidebar').slideToggle();">Каталог товаров</a></li>
                </ul>
                {php} echo $this->navigation()->menu($this->menu)->setPartial('menu3.tpl')->setMinDepth(1)->setMaxDepth(2);{/php}
            {if $zf.params.controller=='catalog' && $zf.params.action=='item'}
                {literal}
                    <script>
                        $(document).ready(function () {
                            var itemCurrentID = {/literal}{$item.id}{literal};
                                        ...
                                        {foreach from=$brand.items item=itemtopsale}
                                            <li class="data-item-{$itemtopsale.id}">
                                                <em><a href="/{$cat_info.subdomain}{iurl assoc.cat_latin=$itemtopsale.cat_latin_single assoc.parent=$itemtopsale.parentname assoc.cat=$itemtopsale.cat_onename assoc.brand=$itemtopsale.brand assoc.item=$itemtopsale.name}"
                                                       {if $itemtopsale.id_availability == 2 || $itemtopsale.id_availability == 3}class="disabled"
                                                       rel="tooltip"{/if} {if $itemtopsale.id_availability == 2}data-original-title="Наличие уточняйте"{/if} {if $itemtopsale.id_availability == 3}data-original-title="Под заказ"{/if}>{$itemtopsale.name}</a></em>
                                                {if $itemtopsale.acttype=='gift'}
                                                    <i class="mini-icon mini-icon-gift" rel="tooltip"
                                                       data-original-title="Подарок"></i>
                                                {/if}
                                                {if $itemtopsale.specprice && ($itemtopsale.bdprice > $itemtopsale.price) && (($itemtopsale.bdprice - $itemtopsale.price)/$itemtopsale.price > 0.01)}
            ...
            {if $banners}
                {foreach from=$banners item=banner}
                    <div class="banner-aside" id="banner_{$banner.id}">
                        <a href="/news/%D0%BC%D0%B5%D0%BD%D1%8F%D0%B9%D1%82%D0%B5-%D0%B1%D0%BE%D0%BD%D1%83%D1%81%D1%8B-%D0%BD%D0%B0-%D1%82%D0%B5%D1%85%D0%BD%D0%B8%D0%BA%D1%83" title="{$banner.name}"><img style="width: 279px;" src='/uploads/{$banner.filename}' alt="{$banner.filename}"/></a>
                    </div>
                {/foreach}
            {/if}
            {if $page=='payment'}
