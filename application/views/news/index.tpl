...
{if $singleNews}
  ...
  <div class="one_news_container category_description no_margin">
    <div class="news_head">
      <h1>{$news.name}</h1>
      {if $startMonth && $endMonth} <div class="action_time">Акция действует с {$startDateArr[2]} {$startMonth} {$startDateArr[0]}г по {$endDateArr[2]} {$endMonth} {$endDateArr[0]}г</div>{/if}
      <!-- Таймер -->
      {literal}
        <script>
          $(function () {
    ...
    {if $news.big_img}
      <div class="sale-image">
        {if !empty($news.inner_url)}
          <a href="{$news.inner_url}"><img src="/uploads/{$news.big_img}" alt="{$news.name|htmlspecialchars}" title="{$news.name|htmlspecialchars}"></a>
        {else}
          <a href="{$news.inner_url}"><img src="/uploads/{$news.big_img}" alt="{$news.name|htmlspecialchars}" title="{$news.name|htmlspecialchars}"></a>
        {/if}
      </div>
...
{else}
  ...
  <div class="h1_holder span-12 no_margin">
    <h1 class="one_news">Акции интернет-магазина <a href="/rss/news"><img src="/images/rss.png" width="20" class="rss-link" /></a></h1>
  </div>                   
  <div class="action-new">
    <div class="action-new__inner">
      {foreach from=$news item=news key=key name=news}
        <a href="/news/{$news.url|mblower}" class="action-new__item{if $news.expired && $news.is_action==1} action-new__end{/if}">
